...
if (isset($_GET["action"]) && $_GET["action"] == "add") {
    ...
    if (isset($_SESSION["cart"][$id_product])) {
        if ($_SESSION["cart"][$id_product] + $num_product <= $stock) {
            ...
            echo "จำนวนสินค้า " . $name . " มีไม่เพียงพอในจำนวนที่คุณเลือก เหลือสินค้าในคลังเพียง: " . $stock;
        }
    } else {
    ...
    dropdown.addEventListener('change', function(e) {
        ...
        if (value === 'P001') {
            ...
            tableSelect.innerHTML = `
                ...
                <?php
                if (!empty($_SESSION["cart"]) && isset($_SESSION["type"])) {
                    ...
                    for ($i = 0; $i < $_SESSION["type"]; $i++) {
                        ...
                        if ($row) {
                            $_SESSION["freeBigSize"] = $randomID; // แก้ตัวแปรให้ถูก
                            echo "<tr>
                                    <td>{$row['name_product']}</td>
                                    <td>0</td>
                                    <td>1</td>
            ...
            tableSelect.innerHTML = `
                ...
                <?php
                if (!empty($smallBookIds) && isset($promotionSmallCount)) {

                    $uniqueSmallBookIds = array_keys($smallBookIds);

                    for ($i = 0; $i < $promotionSmallCount; $i++) {

                        if (!isset($uniqueSmallBookIds[$i])) continue;
